...
/**
 * Manejador global de excepciones (@RestControllerAdvice) para todo el módulo.
 * Traduce cada excepción de negocio/framework a un cuerpo JSON uniforme con
 * timestamp/status/mensaje y el HTTP status correspondiente.
 */
...
public class GlobalExceptionHandler {
    ...
    /** Cotización, usuario o componente inexistente (lanzada desde {@code CotizacionService}) → 404 Not Found. */
    @ExceptionHandler(RecursoNoEncontradoException.class)
    public ResponseEntity<Map<String, Object>> handleNoEncontrado(RecursoNoEncontradoException ex) {
        return construirRespuesta(HttpStatus.NOT_FOUND, ex.getMessage());
    }

    /** Falla de comunicación Feign con ms-usuarios o ms-componentes → 502 Bad Gateway. */
    ...
    /** Falla de validación de Bean Validation (@Valid en CotizacionRequestDTO) → 400 Bad Request con detalle por campo. */
    ...
    /** Violación de restricción de integridad de datos a nivel de BD → 409 Conflict. */
    ...
    /** Parámetro de la URL con tipo incorrecto (ej. ID no numérico) → 400 Bad Request. */
    ...
    /** Cuerpo de la petición JSON mal formado o ilegible → 400 Bad Request. */
    ...
    /** Falta un parámetro requerido en la petición → 400 Bad Request. */
    ...
    /** Cualquier otra excepción no controlada explícitamente → 500 Internal Server Error. */

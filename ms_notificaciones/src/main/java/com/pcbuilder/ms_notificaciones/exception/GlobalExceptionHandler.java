...
/**
 * Manejador global de excepciones del microservicio; traduce cada excepción (de negocio o de
 * Spring) a una respuesta JSON uniforme con timestamp/status/mensaje y el HTTP status correspondiente.
 */
...
public class GlobalExceptionHandler {
    ...
    /** Notificación o usuario referenciado no encontrado ({@link RecursoNoEncontradoException}) → 404. */
    @ExceptionHandler(RecursoNoEncontradoException.class)
    public ResponseEntity<Map<String, Object>> handleNoEncontrado(RecursoNoEncontradoException ex) {
        return construirRespuesta(HttpStatus.NOT_FOUND, ex.getMessage());
    }

    /** Falla de comunicación con ms-usuarios ({@link ErrorComunicacionException}) → 502 Bad Gateway. */
    ...
    /** Falla de validación de Bean Validation en el DTO de request (@Valid) → 400, con el detalle por campo. */
    ...
    /** Violación de restricción de integridad en la base de datos → 409 Conflict. */
    ...
    /** Parámetro de request con tipo incorrecto (ej. ID no numérico) → 400. */
    ...
    /** Cuerpo de la petición mal formado o ilegible (JSON inválido) → 400. */
    ...
    /** Falta un parámetro requerido en la petición → 400. */
    ...
    /** Verbo HTTP no soportado por el endpoint invocado → 405 Method Not Allowed. */
    ...
    /** Cualquier otra excepción no controlada → 500 Internal Server Error, se registra en el log. */
    ...
    /** Arma el cuerpo JSON estándar (timestamp/status/mensaje) para todas las respuestas de error. */
